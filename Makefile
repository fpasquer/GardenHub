...
DEPLOY_SERVICES := gardenhub-nginx gardenhub-api gardenhub-worker gardenhub-consumer gardenhub-notifier gardenhub-scheduler
...
deploy-logs: ## Best-effort diagnostics: container status and recent logs (last 2 minutes)
	...
	-@echo "==> [deploy-logs] gardenhub-notifier (last 2m)"; $(COMPOSE) $(COMPOSE_FILES) logs --since=2m gardenhub-notifier
